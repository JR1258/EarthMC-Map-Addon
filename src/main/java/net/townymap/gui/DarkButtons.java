...
public final class DarkButtons {
     ...
     */
    private static void drawLabel(GuiGraphicsExtractor ctx, Minecraft mc,
        ...
        if (width <= available) {
            ctx.text(mc.font, label, x + (w - width) / 2, textY, color, false);
            return;
        }
        int overflow = width - available;
        double period = Math.max(overflow * PERIOD_PER_SCROLLED_PIXEL, MIN_SCROLL_PERIOD);
        double phase = Math.sin((Math.PI / 2) * Math.cos((Math.PI * 2) * (Util.getMillis() / 1000.0) / period)) / 2.0 + 0.5;
        int offset = (int) Math.round(Mth.lerp(phase, 0.0, overflow));
        // Clip to the body, not the border, so the text never touches the frame.
        ctx.enableScissor(left, y, x + w - LABEL_PAD, y + h);
        try {
            ctx.text(mc.font, label, left - offset, textY, color, false);
        } finally {
            ctx.disableScissor();
        }
    }
